            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 min-w-0 flex-1 flex-wrap">
              ...
              <div className="flex grow items-center gap-2 justify-between">
                <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                  ...
                  {audio.listenCounter !== undefined &&
                    audio.listenCounter > 0 && (
                      <Chip
                        size="sm"
                        variant="flat"
                        color="secondary"
          ...
          <div className="space-y-1 text-sm text-default-500">
            <p className="break-words">
              <span className="font-medium">
                {t("labels.originalFileName")}
              </span>{" "}
              {audio.originalFileName}
            </p>
            <p>
              <span className="font-medium">{t("labels.uploaded")}</span>{" "}
              {formatTimeAgo(new Date(audio.createdAt), locale)}
            </p>
            {audio.lastListenAt && (
              <p>
                <span className="font-medium">{t("labels.lastListened")}</span>{" "}
                {formatTimeAgo(new Date(audio.lastListenAt), locale)}
              </p>
            )}
          </div>
        </CardBody>
