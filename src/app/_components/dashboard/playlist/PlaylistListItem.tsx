        ...
        <CardBody className="gap-2">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 min-w-0 flex-1">
              <h3 className="flex-shrink-0 text-lg font-semibold">
                {playlist.name}
              </h3>
              <div className="flex grow items-center gap-2 justify-between">
                <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                  ...
                  {playlist.listenCounter !== undefined &&
                    playlist.listenCounter > 0 && (
                      <Chip
                        size="sm"
                        variant="flat"
                        color="primary"
                        startContent={<Headphones size={14} />}
                      >
          ...
          <div className="space-y-1 text-sm text-default-500">
            <p>
              ...
              {formatTimeAgo(new Date(playlist.createdAt), locale)}
            </p>
            <p>
              <span className="font-medium">{t("labels.lastUpdated")}</span>{" "}
              {formatTimeAgo(new Date(playlist.updatedAt), locale)}
            </p>
            {playlist.lastListenAt && (
              <p>
                <span className="font-medium">{t("labels.lastListened")}</span>{" "}
                {formatTimeAgo(new Date(playlist.lastListenAt), locale)}
              </p>
            )}
          </div>
        </CardBody>
