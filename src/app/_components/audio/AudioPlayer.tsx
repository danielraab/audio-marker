...
export default function AudioPlayer({
  ...
  const syncRegionsToMarkers = useCallback(
    (list: AudioMarker[], editingId: string | null | undefined) => {
      if (!regionsPlugin.current || !wavesurfer.current) return;

      const existing = regionsPlugin.current.getRegions();
      const markerRegions = existing.filter((r) =>
        r.id.startsWith(markerIdPrefix),
      );
      const wanted = new Set(list.map((m) => markerIdPrefix + m.id));

      // Remove stale
      markerRegions.forEach((r) => {
        if (!wanted.has(r.id)) r.remove();
      });
  ...
  useEffect(() => {
    if (!waveformRef.current || !mounted || !peaksResolved) return;
    ...
    const onRegionCreated = (region: Region) => {
      ...
      if (selectionRegionId.current) {
        const prev = regions
          .getRegions()
          .find((r) => r.id === selectionRegionId.current);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- peaks is read via closure; peaksResolved gates the init
  }, [audioUrl, mounted, peaksResolved]);

  // ── Sync marker regions (no WaveSurfer re-init) ─────────────────────

  useEffect(() => {
    if (isReady && wavesurfer.current && regionsPlugin.current) {
      syncRegionsToMarkers(markers, editingMarkerId);
    }
  }, [markers, isReady, syncRegionsToMarkers, editingMarkerId]);

  // Seek to marker being edited
  useEffect(() => {
    if (isReady && wavesurfer.current && editingMarkerId) {
  ...
  }, [editingMarkerId, isReady, markers]);
  ...
  const playFrom = useCallback((marker: AudioMarker) => {
    ...
    const region = regionsPlugin.current
      ?.getRegions()
      .find((r) => r.id === markerIdPrefix + marker.id);
    if (region) {
      activeRegionId.current = region.id;
      region.play();
    } else {
      wavesurfer.current.seekTo(
        marker.timestamp / wavesurfer.current.getDuration(),
      );
    }
  }, []);
  ...
  useEffect(() => {
    if (isReady && onPlayFromFnReady) onPlayFromFnReady(playFrom);
  }, [isReady, playFrom, onPlayFromFnReady]);

  useEffect(() => {
    if (isReady && onPlayReady) {
      onPlayReady(() => {
  ...
  useEffect(() => {
    if (isReady && onClearRegionReady) onClearRegionReady(clearSelectionRegion);
  }, [isReady, clearSelectionRegion, onClearRegionReady]);
    ...
    <div className="relative w-full">
      ...
      <div className="mb-3 flex w-full flex-col">
        <div className="flex items-center justify-between gap-2 text-lg font-semibold">
          {audioName}
          <Link
            href={`/audios/${audioReadOnlyToken}/listen`}
        ...
        {audioDescription && (
          <p className="text-default-600 mt-1 mb-2 text-sm">
            {audioDescription}
          </p>
        )}
      ...
      <div
        ref={waveformRef}
        className="border-default-200 w-full rounded-lg border p-2"
        style={{ minHeight: "100px", contain: "layout style" }}
      />

      {/* Error */}
      {loadError && (
        <div className="bg-danger-50 border-danger-200 my-4 rounded-lg border p-4">
          <p className="text-danger-600 font-semibold">{t("error.title")}</p>
          <p className="text-danger-500 mt-1 text-sm">{t("error.message")}</p>
          <p className="text-danger-400 mt-2 text-xs">{loadError}</p>
        </div>
      )}
      ...
      <div className="my-4 flex flex-col gap-4 sm:flex-row">
        <div className="flex flex-1 items-center gap-3">
          ...
          <span className="text-default-500 min-w-12 text-sm">
            {t("zoom.label")}
          </span>
          <Slider
            size="sm"
          ...
          />
          <span className="text-default-500 min-w-8 text-xs">{zoomLevel}</span>
        </div>

        <div className="flex flex-1 items-center gap-3">
          <div
            onDoubleClick={handleSpeedReset}
          ...
          />
          <span className="text-default-500 min-w-8 text-xs">
      ...
      {/* Time Display */}
      <div className="text-default-500 flex justify-between text-sm">
