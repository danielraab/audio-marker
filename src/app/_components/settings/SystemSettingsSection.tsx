...
export default function SystemSettingsSection() {
  ...
  const { data: settings, isLoading: isRegStatusLoading } =
    api.admin.systemSettings.getRegistrationStatus.useQuery();
  const { data: headData, isLoading: isHeadLoading } =
    api.admin.systemSettings.getHeadInjection.useQuery();
          ...
          <div className="flex flex-wrap gap-2 items-center justify-between">
            <div className="flex-1">
              <h4 className="text-sm font-medium">
                {t("authSettings.emailRegistration.title")}
              </h4>
              <p className="text-sm text-default-500">
                {t("authSettings.emailRegistration.description", {
                  status: registrationEnabled
                    ? t("authSettings.status.enabled")
                    : t("authSettings.status.disabled"),
                })}
              </p>
        ...
        <CardBody className="gap-4">
          ...
          {setHeadMutation.isSuccess && (
            <p className="text-sm text-success">
              {t("headInjection.saveSuccess")}
            </p>
          )}
          {setHeadMutation.isError && (
            <p className="text-sm text-danger">
              {t("headInjection.saveError")}
            </p>
          )}
        </CardBody>
      </Card>
